# -*- coding: utf-8 -*-
"""
Created on Sun May 20 11:00:36 2018
...
def JetCluster(p,R,A):
    ...
    #calculate the dij and diB matrices
    diB=np.zeros(n)
    dij=np.zeros([n,n])
    for i in range(n):
        pxi=A[i]['px']
    ...
    while len(A)>1:
        ...
        if t1<t2:
            jet.append(A[m1]) #pick out the pseudojet to be a jet
    ...
    if theta2==0:
        '''
        In this case, a pseudojet became a jet without combination
        with any other pseudojets. Thus the pmass here is actually
        the rest mass of this parton, which is neglected in our 
        assumption.
        '''
        pmass=0
    else:
...
be=time.clock()
f=open('njet_p0_R1.0.txt','w')
for i in range(1000):
    FS1=PartonShower3D(A1)
    FS2=PartonShower3D(A2)
    FS=np.concatenate((FS1,FS2))
    nj,pm=JetCluster(0,1.0,FS)
    f.write(str(nj)+'\t'+str(pm)+'\n')
    if i%100==0:
        print(i)
f.close()
